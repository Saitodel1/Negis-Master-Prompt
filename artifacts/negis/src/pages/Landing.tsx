...
type ModalState = 'idle' | 'choice' | 'login' | 'register' | 'departments';
...
export default function Landing() {
  ...
  const handleRegister = async (data: RegisterValues) => {
    ...
    try {
      ...
      const { data: clinic, error: clinicError } = await supabase.from('clinics').insert({ name: data.clinicName, owner_id: userId }).select('id').single();
      if (clinicError) throw clinicError;
      const { error: roleError } = await supabase.from('user_roles').insert({ user_id: userId, clinic_id: clinic.id, role: 'owner' });
  ...
  const circleInner: React.CSSProperties = {
    ...
    alignItems: 'center',
    justifyContent: 'center',
    /* Main surface — matte graphite ceramic */
        ...
        >
          <div style={circleInner}>
            ...
            <span style={{
              ...
              userSelect: 'none',
              position: 'relative',
              /* Engraved feel — very light shadow offset */
      ...
      {modalState !== 'idle' && (
        ...
        >
          ...
          >
            ...
            {/* Departments */}
            {modalState === 'departments' && (
              <DepartmentSelect userRole={userRole} onNavigate={setLocation} onSignOut={() => { closeModal(); signOut(); }} />
            )}
          </div>
        </div>
      )}
  ...
  return (
    <button
      ...
      onMouseEnter={e => {
        ...
        el.style.borderColor = '#DDE5EE';
        el.style.transform = 'translateY(-1px)';
        el.style.boxShadow = '0 4px 12px rgba(15,23,42,0.07)';
      }}
      onMouseLeave={e => {
        const el = e.currentTarget as HTMLButtonElement;
        el.style.background = '#F4F7FB';
        el.style.borderColor = '#E7ECF3';
...
/* ── Department Select ────────────────────────────────────── */
function DepartmentSelect({ userRole, onNavigate, onSignOut }: {
  userRole: string | null; onNavigate: (p: string) => void; onSignOut: () => void;
}) {
  const isManager = userRole === 'owner' || userRole === 'manager';
  const depts = [
    { label: 'Запись',    path: '/booking',    Icon: CalendarDays, always: true },
    { label: 'Ресепшн',   path: '/reception',  Icon: Building2,    always: true },
    { label: 'CRM',       path: '/sales',      Icon: Briefcase,    always: true },
    { label: 'Дашборд',   path: '/dashboard',  Icon: BarChart2,    always: false },
    { label: 'Управление',path: '/admin',       Icon: Settings,     always: false },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      {depts.map(({ label, path, Icon }) => (
        ...
        >
          <Icon size={17} color="#2859C5" strokeWidth={1.75} />
          {label}
        </button>
      ))}
      <button
